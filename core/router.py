"""Thin deterministic safety router; all natural language goes to the agent."""
...
class Router:
    ...
    def _legacy_command(self,text):
        ...
        if any(w.lower() in low for w in session.get("restore_command",[])): return self.sessions.restore(self.executor)+" Рабочий режим активирован."
        ...
        if low in ("сбрось память","забудь диалог"): self.brain.reset_memory(); return "Контекст текущего диалога очищен."
        if low in ("удали всю память","очисти всю память"):
            return self.brain.tools.execute("clear_memory", {}) if self.brain.tools else "Инструменты памяти недоступны."
        return None
    def handle(self,text):
        ...
        return answer or "AI-модуль не подключён. Проверьте OPENROUTER_API_KEY и JARVIS_MODEL."
