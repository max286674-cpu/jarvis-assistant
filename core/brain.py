...
SYSTEM_PROMPT = """Ты — ДЖАРВИС, персональный AI-ассистент пользователя {user}. Ты говоришь на русском языке.
...
- Для голосового режима отвечай кратко, если пользователь не просит подробно.
    - Говори с харизмой: уверенно, с лёгкой иронией, обращайся «сэр» иногда.
    - Будь быстрее: не растягивай ответ, давай суть сразу.
...
class Brain:
    ...
    def _openrouter(self, text: str, model: str) -> str:
        ...
        print(f"[LLM] {model} | токенов: {total_tokens} | цена ~${total_tokens/1000000*0.05:.4f}")
