...
class DuplexSpeaker:
    def __init__(self, voice_cfg: dict):
        self.cfg = voice_cfg
        self._stop = threading.Event(); self._speaking = threading.Event(); self._thread = None
    ...
    def say(self, text: str, blocking: bool = True):
        ...
        if blocking: self._speak(text)
            ...
            self._thread = threading.Thread(target=self._speak, args=(text,), daemon=True); self._thread.start()

    def say_sentences(self, sentences, blocking=False):
        items = [str(x).strip() for x in sentences if str(x).strip()]
        if not items: return
        self.stop(); self._stop.clear()
