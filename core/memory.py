"""Persistent explicit memory on SQLite + FTS5 when available."""
...
class MemoryStore:
    ...
    def add(self, text: str, category: str = "fact") -> None:
        text = (text or "").strip()
        category = (category or "fact").strip()[:40]
        if not text:
            return
        ...
        with sqlite3.connect(self.path) as db:
            ...
            try:
                db.execute("INSERT INTO memories_fts(rowid,text,category) VALUES(?,?,?)", (cur.lastrowid, text, category))
            except sqlite3.OperationalError:
                pass
